...
teacher=load_model('./models/cifar10-vgg13.h5')
...
def create_student(d, num_of_classes):
    set_seed_TF2(100)
    inp=Input(shape=d)
...
def create_kd(d, num_of_classes):
    set_seed_TF2(100)
    inp=Input(shape=d)
    flat_inp=vgg_block(inp)
...
# Regular Student
student=create_student(d,num_of_classes)
...
student_kd=create_kd(d,num_of_classes)
student_kd.fit(train_batches, steps_per_epoch=x_train.shape[0]//s_batch, epochs=s_epoch, callbacks=callbacks, verbose=train_verbose)
y_pred_test=student_kd.predict(x_test)
